...
const VehicleList = () => {
  ...
  const filteredVehicles = vehicles.filter(vehicle => Array.isArray(vehicle.location) && vehicle.location.includes("KEMPAPURA"));

  const quantity = filteredVehicles.map(vehicle =>
    vehicle.location.findIndex(location => location === "KEMPAPURA") !== -1 ?
      vehicle.quantity[vehicle.location.findIndex(location => location === "KEMPAPURA")] :
      ""
  );
